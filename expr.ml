...
let tag (p : 'a program) : tag program =
  ...
  let rec helpE (e : 'a expr) : tag expr =
    ...
    | ELet(binds, body, _) ->
       let let_tag = tag() in
       ELet(List.map (fun (x, topt, b, _) -> let t = tag() in (x, topt, helpE b, t)) binds,
            helpE body,
            let_tag)
    | ELetRec(binds, body, _) ->
       let let_tag = tag() in
       ELetRec(List.map (fun (x, topt, b, _) -> let t = tag() in (x, topt, helpE b, t)) binds,
               helpE body,
               let_tag)
    | EIf(cond, thn, els, _) ->
       let if_tag = tag() in
...
let simple_to_imm simple =
  match simple with
  | Id x -> ImmId(x, ())
  | Num n -> ImmNum(n, ())
  | Bool b -> ImmBool(b, ())
  | _ -> failwith (sprintf "Not an immediate: %s\n" (string_of_simple simple))
...
let rec simple_to_cexpr simple =
  ...
  | Fun(args, body) -> CLambda(args, ACExpr(simple_to_cexpr body), ())
...
let rec cexpr_to_simple_opt cexp =
  match cexp with
    ...
    begin match body with
      | ACExpr(cexp) ->
        begin match cexpr_to_simple_opt cexp with
          | Some(simple_body) -> Some (Fun(args, simple_body))
          | None -> None
        end
      | _ -> None
    end
...
let untag_imm imm = simple_to_imm (imm_to_simple imm)
